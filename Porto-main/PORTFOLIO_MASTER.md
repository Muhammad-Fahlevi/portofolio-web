# PORTFOLIO_MASTER.md

# Portofolio Muhamad Sahal Ansar

Versi: 1.0

## Tentang Dokumen

Dokumen ini menjadi pedoman utama dalam membangun website portofolio Muhamad Sahal Ansar.

Website ini **bukan website portofolio biasa**. Website harus memberikan pengalaman seperti **Spotify Wrapped** atau **Instagram Stories**, di mana setiap layar adalah satu cerita.

## Tujuan

- Memperkenalkan diri secara profesional.
- Menampilkan perjalanan belajar dan karier.
- Menunjukkan proyek unggulan.
- Membangun personal branding sebagai Frontend Developer.

## Konsep

- Fullscreen story.
- Tidak menggunakan long scrolling.
- Navigasi tap kiri/kanan, swipe, keyboard.
- Progress bar di bagian atas.
- Satu slide = satu cerita.

## Tentang Saya

**Nama:** Muhamad Sahal Ansar

**Domisili:** Banjarmasin, Kalimantan Selatan

**Pendidikan:** D3 Teknik Informatika – Politeknik Negeri Banjarmasin

**Fokus:**
- Frontend Development
- Laravel
- UI/UX
- Responsive Web

## Ringkasan

Saya adalah mahasiswa D3 Teknik Informatika yang memiliki ketertarikan pada pengembangan website modern. Saya menyukai proses mengubah ide menjadi antarmuka yang bersih, responsif, dan nyaman digunakan.

## Perjalanan

### 2023
- Masuk Politeknik Negeri Banjarmasin.
- Belajar HTML, CSS, JavaScript, PHP.

### 2024
- Membangun proyek perkuliahan.
- Belajar Laravel.
- Aktif di organisasi.

### 2025
- Magang Frontend Developer di Neumedira Indonesia.
- Ketua Umum KSR PMI Unit Poliban.
- Mengembangkan Tugas Akhir Sistem Informasi Rental Mobil dengan Dashboard Monitoring.

## Keahlian

### Frontend
- HTML
- CSS
- Tailwind CSS
- JavaScript
- Laravel Blade

### Backend
- PHP
- Laravel

### Database
- MySQL

### Tools
- Git
- GitHub
- VS Code
- Figma

### Soft Skill
- Leadership
- Teamwork
- Communication
- Problem Solving

## Pengalaman

### Neumedira Indonesia
Frontend Developer Intern
- Dashboard Administration
- Laravel Blade
- Tailwind CSS
- Responsive Design
- Git & GitHub

### KSR PMI Unit Poliban
Ketua Umum
- Memimpin organisasi
- Koordinasi divisi
- Pengambilan keputusan

## Proyek Utama

### Sistem Informasi Rental Mobil
- Dashboard Monitoring
- Booking Online
- Manajemen Member
- Tracking Kendaraan
- Laravel
- Tailwind CSS
- MySQL

## Struktur Website

1. Pembuka
2. Tentang Saya
3. Perjalanan
4. Keahlian
5. Pengalaman
6. Proyek
7. Pencapaian
8. Penutup

## Gaya Visual

- Dark mode
- Minimalis
- Premium
- Storytelling
- Banyak whitespace
- Tipografi besar

## Animasi

- Fade
- Slide
- Scale
- Counter
- Text Reveal
- Progress Bar

## Hindari

- Long scrolling
- Template AI
- Dashboard admin
- Hero section generik
- Glassmorphism berlebihan

## Tujuan Akhir

Pengunjung harus merasa:

> "Saya tidak hanya melihat portofolio Sahal, tetapi ikut merasakan perjalanan dan prosesnya."
